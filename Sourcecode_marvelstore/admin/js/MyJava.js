...
function Validate_admin(){ //validate cho phần thêm thành viên admin
	if(blur_name() && blur_phone() && blur_email() && blur_repass() && blur_pass()){
		return true;
	}else{
		alert('Dữ liệu nhập vào chưa đúng, yêu cầu kiểm tra lại !');
		return false;
...
//Click xem chi tiết nội dung phản hồi khách hàng,show modal
$(document).on('click', '.btn_detail_contact', function(e) {
	e.preventDefault();
	var id=$(this).val();
	$.get('server/contact/show-detail-contact.php',{id:id},function(data) {
		$("#exampleModal").html(data);
	});
});

//Click xóa đơn hàng
$(document).on('click', '.btn_del_contact', function(e) {
	e.preventDefault();
	var id=$(this).val();
	var check=confirm("Bạn có muốn xóa phản hồi này không ?");
	if(check){
		$.get('server/contact/del-contact.php',{id:id}, function(data) {
			alert(data);
			$("#tbl_contact_boxout").load(" #tbl_contact_boxin");
		});
	}
});
...
// ứng với ID của đơn đó => search ra đơn hàng
	$(".setSS_order").click(function(e) {
		...
		$.get('server/order/setSS_order_byID.php',{id:id}, function(data) {
		});
	});
...
//JS cho chức năng sắp xếp bảng thống kê rating
$(document).on('change', '#sort_report_raiting', function(e) {
	e.preventDefault();
	var ss=$("#sort_report_raiting").val();
	$.get('server/status/sort-report-raiting.php',{ss:ss}, function(data) {
		$("#report_raiting_box").load(" #table_report_raiting");
	});
});

//Click hiển thị những đơn hàng Đang chờ xử lý, phần Status báo cáo thống kê
$("#odering").click(function(e) {
	$.get('server/order/setSS_order_bySTT1.php', function(data) {
	});
});
...
//Click xóa bài viết
$(document).on('click','.del_news', function(e) {
	e.preventDefault();
	var id=$(this).val();
	var check=confirm("Bạn có muốn xóa bài viết này không ?");
	if(check){
		$.get('server/news/del-news.php',{id:id}, function(data) {
			alert(data);
			$("#tbl_news_boxout").load(" #tbl_news_boxin");
		});
