...
//Xoá 1 bài tin tức
            public function Del_News($id_news){
...
//Lấy ra thông tin của 1 bài báo dựa theo ID
            public function getNew_ID($id_news){
...
//Thêm mới 1 quản trị viên bảng tbl_admin
            protected function add_Admin($name_admin,$phone,$email,$stt_admin,$pass){
                $sql="INSERT INTO tbl_admin (name_admin,phone,email,stt_admin,password) VALUES (:name_admin,:phone,:email,:stt_admin,:pass)";
                $pre=$this->pdo->prepare($sql);
...
//Lấy ra noti của 1  sản phầm bảng tbl_noti_product theo ID
        ...
        //Tìm ra tất cả đơn hàng chưa được xử lý bảng tbl_order
        public function get_order_stt1(){
            $sql = "SELECT * FROM tbl_order WHERE stt=1";
            $pre = $this->pdo->prepare($sql);
            $pre->execute();
            return $pre->fetchAll(PDO::FETCH_ASSOC);
        }
...
//Lấy ra tổng số sản phẩm tìm kiếm để thống kê đánh giá
        public function report_raiting_product_all($key_search){
            if($key_search==false){
                $key_search='%%';
            }
            $sql="SELECT (SELECT COUNT(star) FROM tbl_review,tbl_product WHERE tbl_review.id_product=tbl_product.id_product AND star=1 AND (tbl_product.name_product LIKE :key_search OR tbl_product.id_product LIKE :key_search)) as tongsolan1s,(SELECT COUNT(star) FROM tbl_review,tbl_product WHERE tbl_review.id_product=tbl_product.id_product AND star=2 AND (tbl_product.name_product LIKE :key_search OR tbl_product.id_product LIKE :key_search)) as tongsolan2s,(SELECT COUNT(star) FROM tbl_review,tbl_product WHERE tbl_review.id_product=tbl_product.id_product AND star=3 AND (tbl_product.name_product LIKE :key_search OR tbl_product.id_product LIKE :key_search)) as tongsolan3s,(SELECT COUNT(star) FROM tbl_review,tbl_product WHERE tbl_review.id_product=tbl_product.id_product AND star=4 AND (tbl_product.name_product LIKE :key_search OR tbl_product.id_product LIKE :key_search)) as tongsolan4s,(SELECT COUNT(star) FROM tbl_review,tbl_product WHERE tbl_review.id_product=tbl_product.id_product AND star=5 AND (tbl_product.name_product LIKE :key_search OR tbl_product.id_product LIKE :key_search)) as tongsolan5s,COUNT(star) as tong
FROM tbl_review,tbl_product
WHERE tbl_review.id_product=tbl_product.id_product AND (tbl_product.name_product LIKE :key_search OR tbl_product.id_product LIKE :key_search)";
            $pre = $this->pdo->prepare($sql);
            $pre->bindParam(':key_search', $key_search);
            $pre->execute();
            return $pre->fetchAll(PDO::FETCH_ASSOC);
...
//Lấy ra thống kê lượt đánh giá của từng sản phẩm    
        public function report_raiting_product_detail($key_search){
            if($key_search==false){
                $key_search='%%';
            }              
            $sql="SELECT tbl_review.id_product as idsp,tbl_product.name_product,tbl_product.img,(SELECT COUNT(star) FROM tbl_review WHERE star=1 AND id_product=idsp) as solan1s,(SELECT COUNT(star) FROM tbl_review WHERE star=2 AND id_product=idsp) as solan2s,(SELECT COUNT(star) FROM tbl_review WHERE star=3 AND id_product=idsp) as solan3s,(SELECT COUNT(star) FROM tbl_review WHERE star=4 AND id_product=idsp) as solan4s,(SELECT COUNT(star) FROM tbl_review WHERE star=5 AND id_product=idsp) as solan5s,COUNT(star) as tong,AVG(star) as AVG
FROM tbl_review,tbl_product
WHERE tbl_product.id_product=tbl_review.id_product AND (tbl_product.name_product LIKE :key_search OR tbl_product.id_product LIKE :key_search)
GROUP BY idsp";
            $pre = $this->pdo->prepare($sql);
            $pre->bindParam(':key_search',$key_search);
            $pre->execute();
...
//Lấy ra thống kê lượt đánh giá của từng sản phẩm  , có limit phân trang      
        public function report_raiting_product_detail_limit($key_search,$from,$row){
            if($key_search == false){
                $key_search='%%';
            }
            if(!isset($_SESSION['sort_raiting']) || (isset($_SESSION['sort_raiting'])&&$_SESSION['sort_raiting']=='count_rating_DESC')){
                $sort='ORDER BY tong DESC';
            }else if(isset($_SESSION['sort_raiting'])&&$_SESSION['sort_raiting']=='count_rating_ASC'){
                $sort='ORDER BY tong ASC';
            }else if(isset($_SESSION['sort_raiting'])&&$_SESSION['sort_raiting']=='AVG_raiting_ASC'){
                $sort='ORDER BY AVG ASC';
            }else if(isset($_SESSION['sort_raiting'])&&$_SESSION['sort_raiting']=='AVG_raiting_DESC'){
...
FROM tbl_review,tbl_product
WHERE tbl_product.id_product=tbl_review.id_product AND (tbl_product.name_product LIKE'$key_search' OR tbl_product.id_product LIKE '$key_search')
GROUP BY idsp ".$sort." LIMIT $from,$row";
            $pre = $this->pdo->query($sql);
            return $pre->fetchAll(PDO::FETCH_ASSOC);
        }

//Lấy ra danh sách  tất phản hồi của khách hàng- bảng tbl_contact
            public function get_Contact($ss_search){
                if($ss_search==false){
                    $sql="SELECT * FROM tbl_contact ORDER BY created DESC";
                    $pre=$this->pdo->prepare($sql);
                }else{
                    $sql="SELECT * FROM tbl_contact WHERE (name LIKE '$ss_search' OR email LIKE '$ss_search' OR phone LIKE '$ss_search' OR created LIKE '$ss_search') ORDER BY created DESC";
                    $pre=$this->pdo->prepare($sql);
                    $pre->bindParam(':ss_search',$ss_search);
                }
                $pre->execute();
...
//Lấy ra danh sách  tất phản hồi của khách hàng,có phân trang- bảng tbl_contact
            public function get_Contact_limit($form,$row,$ss_search){
                if($ss_search==false){
                    $sql="SELECT * FROM tbl_contact ORDER BY created DESC LIMIT $form,$row";
                }else{
                    $sql="SELECT * FROM tbl_contact WHERE (name LIKE '$ss_search' OR email LIKE '$ss_search' OR phone LIKE '$ss_search' OR created LIKE '$ss_search') ORDER BY created DESC LIMIT $form,$row";
                }
                $pre=$this->pdo->query($sql);
                return $pre->fetchAll(PDO::FETCH_ASSOC);
            }

//Lấy ra thông tin đầy đủ của 1 phản hồi của khách hàng dựa theo ID
            public function getDetail_Contact_Id($id_contact){
                $sql="SELECT * FROM tbl_contact WHERE id_contact=:id_contact";
                $pre=$this->pdo->prepare($sql);
...
//Xoá 1 contact
            public function Del_Contact($id_contact){
                $sql="DELETE FROM tbl_contact WHERE id_contact=:id_contact";
                $pre=$this->pdo->prepare($sql);
                $pre->bindParam(':id_contact',$id_contact);
                $pre->execute();
                return $pre;            
