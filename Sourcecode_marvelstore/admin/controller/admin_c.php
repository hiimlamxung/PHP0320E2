...
//Lấy hàm Del_News($id_news) cho ajax
    public function Del_News($id_news){
        return $this->ad->Del_News($id_news);
    }
//Lấy hàm getDetail_Contact_Id($id_contact) cho ajax
     public function getDetail_Contact_Id($id_contact){
        return $this->ad->getDetail_Contact_Id($id_contact);
    }
//Lấy hàm Del_Contact($id_contact) cho ajax
    public function Del_Contact($id_contact){
        return $this->ad->Del_Contact($id_contact);
    }
//Lấy hàm getNew_ID($id_news) cho ajax
    public function getNew_ID($id_news){
        return $this->ad->getNew_ID($id_news);
    }
    ...
    // echo '</pre>';
     ...
     switch ($views) {
            ...
            switch ($action) {
                ...
                $order = $this->ad->get_order_stt1();//thống kê tất cả order
            ...
            // echo '</pre>';
             include_once "views/status/status.php";
             break;
...
case 'report-raiting': // Thống kê đánh giá sản phẩm
  if(isset($_POST['submit_report_raiting'])){
        $key_search=trim($_POST['key_search']);
        $_SESSION['report_nameproduct_raiting']='%'.$key_search.'%';        
    }
    if(isset($_SESSION['report_nameproduct_raiting'])){
        $rs_all=$this->ad->report_raiting_product_all( $_SESSION['report_nameproduct_raiting']);
        $rs_detail=$this->ad->report_raiting_product_detail($_SESSION['report_nameproduct_raiting']);
        $row=10;
        $number=count($rs_detail);
        $pagination=ceil($number/$row);
        if(isset($_GET['pages']) && $_GET['pages']<=$pagination && $_GET['pages']>=1){
       ...
       $from=($pages-1)*$row;
       $rs_detail_limit=$this->ad->report_raiting_product_detail_limit($_SESSION['report_nameproduct_raiting'],$from,$row);
   }else{
    $rs_all=$this->ad->report_raiting_product_all(false);
    $rs_detail=$this->ad->report_raiting_product_detail(false);
    $row=10;
    $number=count($rs_detail);
    $pagination=ceil($number/$row);
    if(isset($_GET['pages']) && $_GET['pages']<=$pagination && $_GET['pages']>=1){
   ...
   $from=($pages-1)*$row;
   $rs_detail_limit=$this->ad->report_raiting_product_detail_limit(false,$from,$row);
         // echo "<pre>";
         // print_r($rs_detail_limit);
         // echo "</pre>";
}
$current = $_GET['pages'];
  include_once "views/status/report-raiting.php";
  break;
...
//Thêm mới sản phẩm
   ...
   break;
    		...
    		if(isset($_POST['edit_user'])){//Khi bấm submit sửa khách hàng
              ...
              if($check_phone!=0 && $phone!=$_SESSION['phone_user']){
                 echo "<script>alert('Số điện thoại đã tồn tại, không sửa được !');</script>";
             }else{
    ...
    break;
     ...
     if(isset($_POST['add_admin'])){
        ...
        if($check_phone!=0){
            ...
            $add=$this->ad->add_Admin($name_admin,$phone,$email,$stt_admin,$pass);
            if($add){
...
case 'contact':
             if(isset($_POST['search_contact'])){
                 $_SESSION['key_contact']='%'.$_POST['key_contact'].'%';
             }

             if(!isset($_SESSION['key_contact'])){
                 $rs=$this->ad->get_Contact(false);
            //                  echo '<pre>';
            // print_r($rs);
            // echo '</pre>';
                 $row=10;
                ...
                $form=($pages-1)*$row;
                $rs=$this->ad->get_Contact_limit($form,$row,false);
            }
            else{
             $rs=$this->ad->get_Contact($_SESSION['key_contact']);
             $row=10;
             $number=count($rs);  
             $pagination=ceil($number/$row);
             if(isset($_GET['pages']) && $_GET['pages']<=$pagination && $_GET['pages']>=1){
            ...
            $form=($pages-1)*$row;
            $rs=$this->ad->get_Contact_limit($form,$row,$_SESSION['key_contact']);
        }
        $current = $_GET['pages'];
