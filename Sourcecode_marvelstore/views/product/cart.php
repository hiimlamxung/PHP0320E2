...
<div id="table-box-cart" style="min-height: 300px">
	<div id="cart-table">
		...
		if (!isset($_SESSION['cart']) || empty($_SESSION['cart'])) {
			...
			?>
			<h4 class="col-md-6 col-md-push-3 col-xs-10 col-xs-push-2" style="color: red;">Hiện tại chưa có sản phẩm nào!</h4>
			<?php 
		}else
		{
			?>
										...
										<tbody>
											...
											foreach ($_SESSION['cart'] as $key => $cart) {
												...
												<tr>
													...
													<td>
														<input type="number" style="width: 80px;" value="<?php echo $cart['qty'] ?>" class="form-control" min="1" max="<?php echo $cart['quantity']; ?>" id="<?php echo $cart['id_product'] ?>" onchange="updatecart(<?php echo $cart['id_product']; ?>);" onkeypress="return onlyNum();" >
													</td>
													<td><input style="width: 80px;" type="text" readonly="" id="quantity<?php echo $cart['id_product']; ?>" class="form-control" value="<?php echo $cart['quantity'] - $cart['qty']; ?>"></td>
													<?php 
													if ($cart['discount'] <= 0) {
