...
//UPDATE SS cart của 1 user vào tbl_data_user
	...
	//Update lại thông tin của từng sản phẩm trong ss cart. -Trường hợp sản phẩm lỡ bị thay đổi thông tin trước đó do admin
		public function Update_product_info_inSScart($ss_cart){
			foreach ($ss_cart as $key => $value) {
				$memmory_qty=$value['qty'];
				$get_product_info=self::getProduct_Id_SS($key);
				$get_product_info=self::add_discount_SS($get_product_info);
				if(count($get_product_info)==0){
					unset($ss_cart[$key]);
				}else{
					$ss_cart[$key]=$get_product_info;
					$ss_cart[$key]['qty']=$memmory_qty;
				}
			}
			return $ss_cart;
		}
