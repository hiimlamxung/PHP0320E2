...
class Product_c extends product_m
{
	...
	//Lấy hàm add_data_cart($data_cart,$id_user) cho ajax
	public function edit_data_cart($data_cart,$id_user){
		return $this->pro->edit_data_cart($data_cart,$id_user);
	}
	//Lấy hàm get_datauser_ID($id_user) cho ajax
	public function get_datauser_ID($id_user){
		return $this->pro->get_datauser_ID($id_user);
	}
	//Lấy hàm Update_product_info_inSScart($ss_cart) cho ajax
	public function Update_product_info_inSScart($ss_cart){
		return $this->pro->Update_product_info_inSScart($ss_cart);
	}
...
//Thiết lập sản phẩm đã xem qua
	...
	public function typeProduct(){
		...
		switch ($method) {
			...
			$row=12;//Số sản phẩm, tin.. trên 1 trang
...
//Chức năng sắp xếp lại danh sách sản phẩm theo yêu cầu.(Thực sự cái đoạn này k cần thiết vì đã có trong truy vấn bên product_m,nhưng phải bổ sung vào thì mục sản phẩm hot mới sắp xếp được)
		if(!isset($_SESSION['sort']) || (isset($_SESSION['sort'])&&$_SESSION['sort']=='id_product_ASC')){
			array_multisort(array_column($rs, 'id_product'), SORT_ASC, $rs);
		}else if(isset($_SESSION['sort'])&&$_SESSION['sort']=='id_product_DESC'){
			array_multisort(array_column($rs, 'id_product'), SORT_DESC, $rs);
		}else if(isset($_SESSION['sort'])&&$_SESSION['sort']=='name_product_ASC'){
			array_multisort(array_column($rs, 'name_product'), SORT_ASC, $rs);
		}else if(isset($_SESSION['sort'])&&$_SESSION['sort']=='name_product_DESC'){
			array_multisort(array_column($rs, 'name_product'), SORT_DESC, $rs);
		}else if(isset($_SESSION['sort'])&&$_SESSION['sort']=='sale_price_ASC'){
			array_multisort(array_column($rs, 'discount_price'), SORT_ASC, $rs);
