...
//Lấy hàm get_datauser_ID($id_user)	cho ajax
	public function get_datauser_ID($id_user){
		return $this->info->get_datauser_ID($id_user);
	}
		...
		switch ($method) {
			...
			if(isset($_POST['submit_login'])){ //Đăng nhập bình thường thì
				...
				if(count($rs)!=1){
					...
					if(isset($_SESSION['cart']) && !isset($_COOKIE['id_user']) && !isset($_COOKIE['name_user'])){
					...
					foreach ($rs as $key => $value) {
						...
						$get_data_user=$this->info->get_datauser_ID($value['id_user']);
...
//------------------Đăng nhập FB
			...
			$facebook = new \Facebook\Facebook([
				'app_id'      => '3440048582721380',
				'app_secret'     => 'a4866e4cccefc455823f64f84ad3f2c0',
 ...
 // Get login url
    ...
    $facebook_login_url = $facebook_helper->getLoginUrl('http://php0320e2-3.itpsoft.com.vn/info/login', $facebook_permissions);
...
case 'logout':
setcookie('id_user','',(time()-999999999999999999999999999999999999999),'/');
setcookie('name_user','',(time()-999999999999999999999999999999999999999),'/');
setcookie('user_imageFB','',(time()-999999999999999999999999999999999999999),'/');
...
//Thêm dữ liệu vào bảng tbl_detail_order
		...
		//gửi mail 
			...
			<img src="http://php0320e2-3.itpsoft.com.vn/images/logo.webp">
			...
			$data.='<table class="table table-hover" border="1px" style="border-collapse: collapse;">
			<thead>
			<tr>
			<th>Tên sản phẩm</th>
			...
			foreach($_SESSION['cart'] as $key => $value){
				$data.='<tr>
				<td  style="text-align:center;font-style:italic;color:gray;">'.$value['name_product'].'</td>
				<td style="text-align:center;width:100px;position:relative;"><img src="http://php0320e2-3.itpsoft.com.vn/'.$value['img'].'" style="position:absolute;width:85%;object-fit: scale-down;" alt="" ></td>
				<td  style="text-align:center;width:80px;">'.$value['qty'].'</td>
				<td  style="text-align:center;">'.number_format($value['price']).' đ</td>
				';if ($value['discount'] <= 0) {
					$data.='<td style="text-align:center;">'.number_format($value['price']).' đ</td>';
				}else{
					$data.='<td style="text-align:center;">'.number_format($value['discount_price']).' đ</td>';
				}
				$data.='</tr>
				';
			}
			...
			<tr>
			<td colspan="5">Tổng cộng: '.number_format($_SESSION['TOTAL_ORDER']).' đ</td>
			</tr>

			</tbody>
			</table>
			<h3>2. Thông tin khách hàng</h3>
			<p>_ Tên khách hàng: '.$name.'.</p>
			<p>_ Địa chỉ nhận hàng: '.$address.'.</p>
			<p>_ Trên đây là các thông tin về đơn hàng Anh/Chị vui lòng xác nhận lại để đơn hàng được giao theo đúng tiến độ.</p>
			<p>_ Một lần nữa, trân trọn cảm ơn Anh/Chị đã tin tưởng và đồng hành cùng Marvel Store. hy vọng hệ thống sẽ được tiếp đón Anh/Chị trong thời gian tới!</p>
			<p>_ Mọi thắc mắc xin liên hệ tới số điện thoại: <span style="color: red; font-size: 17px">0357545556 - 0398762441.</span> Hoặc truy cập địa chỉ: <span><a href="http://php0320e2-3.itpsoft.com.vn/info/contact" target="_blank" style="font-size: 17px;text-decoration:none;font-style:italic;">php0320e2-3.itpsoft.com.vn/info/contact</span></a></p>
			<br>
			<h4 style="color: gray;font-style:italic;"><i>Trân trọng.</i></h4>
			<h4 style="color: gray;font-style:italic;"><i>Marvel Store</i></h4>
			</div>
			</div>								
			</div>
		...
		echo "<script>window.location.href = 'info/thankyou';</script>";
		} catch (Exception $e) {
...
//Thêm dữ liệu vào bảng tbl_detail_order
		...
		unset($_SESSION['cart']);
		unset($_SESSION['total']);
		$edit_data_cart=$this->info->edit_data_cart($_SESSION['cart'],$_COOKIE['id_user']);
	}
	...
	if(isset($_POST['submit_forget'])){
		...
		if($check_email==0){
			...
			$resetpass_url='http://php0320e2-3.itpsoft.com.vn/index.php?page=info&method=reset-pass&email='.$email.'&code='.$verification_code;
  ...
  *{
	...
	case 'news': //Trang hướng dẫn mua hàng
	$get_news = $this->info->get_new();
	$get_news = $this->info->add_Url_name_news($get_news);

	$get_news2 = $this->info->get_new2();
	$get_news2 = $this->info->add_Url_name_news($get_news2);

	//Lấy ra các sản phẩm hot cho trang tin tức
	$product_rand=$this->info->get_product_rand();
	$product_rand=$this->info->add_discount($product_rand);
	$product_rand=$this->info->add_Url_name($product_rand);
	include_once "views/information/news.php";
	// echo '<pre>';
	// 		print_r($product_hot);
	break;
	...
	$get_news = $this->info->get_new_ID($id);
	$get_news = $this->info->add_Url_name_news($get_news);
	
	$get_all_news=$this->info->get_new();
	$get_all_news=$this->info->add_Url_name_news($get_all_news);
	// echo '<pre>';
	// 		print_r($get_news);
	// echo '</pre>';

	include_once 'views/information/detail-news.php';
	break;
	...
	case 'thankyou':
		include_once "views/information/thankyou.php";
		break;
