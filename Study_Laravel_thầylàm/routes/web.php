...
Route::group(['prefix' => 'db'], function() {
    Route::get("db-list-member", function(){
		...
		return view('pages.DB.db_list_member', ['rs' => $rs]);
		/*
			dd($rs);
			echo "<pre>";
			print_r($rs);
    ...
    Route::get('insert-member', function(){ // hiển thị ra form thêm
    	...
    	return view('pages.DB.db_add_member', ['rs' => $rs]);
    ...
    Route::get('edit-member/{id}', function($id) {
        ...
        return view('pages.DB.db_edit_member', ['id' => $id, 'rs' => $rs, 'member' => $rs_member]);	
    })->name('edit_member');
    ...
    Route::post('db-list-member', function(Request $request) {
        ...
        $count = count($rs);
        return view('pages.DB.db_list_member', ['rs' => $rs, 'key' => $key, 'count' => $count]);
...
Route::group(['prefix' => 'dqb'], function() {
    Route::get('list-member', function() {
     ...
     //                 dd($users); // Tạo biểu thức thô
    	...
    	$rs = DB::table('member')
    				...
    				->paginate(5);
    	$count_rs = count($rs);
        return view('pages.DQB.dqb_list_member', ['rs' => $rs, 'count_rs' => $count_rs]);
    ...
    Route::get("insert-member", function(){
    	...
    	return view('pages.DQB.dqb_insert_member', ['rs' => $rs]);
    ...
    Route::get('edit-member/{id}', function($id){
    	...
    	return view('pages.DQB.dqb_edit_member', ['rs' => $rs, 'member' => $member]);
    ...
    Route::post('list-member', function(Request $request) {
        ...
        $count = count($rs);
        $count_rs = count($rs);
        return view('pages.DQB.dqb_list_member', ['rs' => $rs, 'key' => $key, 'count' => $count, 'count_rs' => $count_rs]);
    });
...
Route::group(['prefix' => 'orm'], function(){
	Route::get('list-member', function() {
	    ...
	    //dd($select);
	 	...
	 	$count_rs = count($rs);
	    return view('pages.ORM.orm_list_member', ['rs' => $rs, 'count_rs' => $count_rs]);
	})->name('orm_list_member');

	Route::get('insert-member', function() {
		$rs = Faculty::all();
	    return view('pages.ORM.orm_insert_member', ['rs' => $rs]);
	...
	Route::get('edit-member/{id}', function($id) {
	    ...
	    $rs = Faculty::all();
	    return view('pages.ORM.orm_edit_member', ['rs' => $rs, 'member' => $member]);
	...
	Route::post('list-member', function(Request $request){
		...
		$count_rs = count($rs);
		$count = count($rs);
		return view('pages.ORM.orm_list_member', ['key' => $key, 'rs' => $rs, 'count_rs' => $count_rs, 'count' => $count]);
