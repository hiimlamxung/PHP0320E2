...
<section class="row text-center placeholders">
    <div class="col-6 col-sm-6 placeholder">
       <a href="{{ route('db_list_member') }}"> <button class="btn btn-primary">Danh sách</button></a>
    </div>
    <div class="col-6 col-sm-6 placeholder">
       <a href="{{ route('db_insert_member') }}"><button class="btn btn-warning">Thêm mới</button></a>
    </div>
</section>
    ...
    <tbody>
        ...
        @foreach ($rs as $value)
            <tr>
                ...
                <td>{{$value->addres}}</td>
                <td colspan="2">
                    <a href="{{ route('edit_member', $value->id) }}"><button class="btn btn-primary">Edit</button></a>
                    <a onclick="return confirm('Bạn có muốn xóa học viên này không? ');" href="{{ route('db_delete_member', $value->id) }}"><button class="btn btn-danger">Delete</button></a>
                </td>
            </tr>
        @endforeach
    </tbody>
